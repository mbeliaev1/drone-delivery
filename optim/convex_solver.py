...
class net_solver():
    def __init__(self, net_params):
        ...
        self.matB, self.matC, self.matS = init_C_ops(
                                        self.G,
                                        self.m,
                                        self.n,
                                        self.paths,
        ...
        # initialize true operator for solution interpretation
        _, _, _, self.mat_true_S = init_ops(
                                        self.G,
                                        self.m,
                                        self.n,
                                        self.paths,
    ...
    def _init_gurobi(self):
        ...
        '''
        # the only difference with QP formulation is replacing matrix S with scaled matrix B
        # main quadratic term 
    ...
    def _convert_sol(self):
        ...
        sol_dict['flow_st'] = self.mat_true_S@sol_dict['X']
    ...
    def visualize_sol(self, lsol_dict, rsol_dict, fig_params, save_path):
        ...
        res_str += r'''        \toprule'''
        res_str += r'''		$\textit{setup}$ & $\beta=1$ & $\beta=0$\\'''
        res_str += r'''  		\cmidrule(r){1-3}'''
        res_str += r'''  		& \multicolumn{2}{c}{latency (\textit{min})}\\'''
        res_str += r'''  		\cmidrule(r){1-3}'''
        res_str +=    '     \\textit{truck} &%.1f&%.1f'%(lsol_dict['latency_truck'],rsol_dict['latency_truck']) + '\\\\'
        res_str +=    '     \\textit{drone} &%.1f&%.1f'%(lsol_dict['latency_drone'],rsol_dict['latency_drone']) + '\\\\'
        res_str +=    '     \\textit{parcel} &%.1f&%.1f'%(lsol_dict['avg_p_l'],rsol_dict['avg_p_l']) + '\\\\'
        res_str +=    '     \\textit{societal} &%.1f&%.1f'%(lsol_dict['avg_s_l'],rsol_dict['avg_s_l']) + '\\\\'
        res_str += r'''		\cmidrule(r){1-3}'''
        res_str += r'''		& \multicolumn{2}{c}{price ($\frac{dollars}{hour}$)}\\'''
        res_str += r'''		\cmidrule(r){1-3}'''
        res_str +=    '     \\textit{truck} &%.1f&%.1f'%(lsol_dict['price_truck'],rsol_dict['price_truck']) + '\\\\'
        res_str +=    '     \\textit{drone} &%.1f&%.1f'%(lsol_dict['price_drone'],rsol_dict['price_drone']) + '\\\\'
        # res_str +=    '     \\textit{parcel} &%.1f&%.1f'%(lsol_dict['price'],rsol_dict['price']) + '\\\\'
        res_str += r'''		\cmidrule(r){1-3}'''
